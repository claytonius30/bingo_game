...
$(function () {
  ...
  if (window.location.href.indexOf('player.html') > -1 || window.location.href.indexOf('.html') === -1) {
    ...
    $('#joingame').on('click', function () {
      ...
      if (playerNameInput !== "" && hostNameInput !== "") {
        ...
        // $.post('http://bingo.consulogic.org/player/join-game', { playerName: playerNameInput, hostName: hostNameInput }, function (data) {
  ...
  // This block handles the dynamically loaded player card page  
    ...
    // let socket = new WebSocket('ws://bingo.consulogic.org/websocket-endpoint');
    // let socket2 = new WebSocket('ws://bingo.consulogic.org/draw-win-endpoint');
    // let socket3 = new WebSocket('ws://bingo.consulogic.org/win-message-endpoint');
    // let socket4 = new WebSocket('ws://bingo.consulogic.org/game-end-endpoint');
    ...
    canvas.addEventListener('click', (event) => {
      ...
      // $.get(`http://bingo.consulogic.org/player/get-draw?hostName=${hostName}&playerName=${playerName}`, function (data1) {
      $.get(`http://localhost:8080/player/get-draw?hostName=${hostName}&playerName=${playerName}`, function (data1) {
        ...
        // $.get(`http://bingo.consulogic.org/player/bingo-card?hostName=${hostName}&playerName=${playerName}`, function (data2) {
        $.get(`http://localhost:8080/player/bingo-card?hostName=${hostName}&playerName=${playerName}`, function (data2) {
          ...
          if (clickedCol === convLet && bingoNumbers[clickedRow][clickedCol] === numby) {
            ...
            // fetch(`http://bingo.consulogic.org/player/update-card?hostName=${hostName}&playerName=${playerName}`, {
    ...
    $('#bingobutton').on('click', function () {
      // $.get(`http://bingo.consulogic.org/player/verify-bingo?hostName=${hostName}&playerName=${playerName}`, function (data) {
    ...
    function getCard(hostName, playerName) {
      // $.get(`http://bingo.consulogic.org/player/bingo-card?hostName=${hostName}&playerName=${playerName}`, function (data) {
...
// ******************** Used prior to web sockets
    ...
    function updateDrawnNumber(hostName, playerName) {
      // $.get(`http://bingo.consulogic.org/player/get-draw?hostName=${hostName}&playerName=${playerName}`, function (data) {
